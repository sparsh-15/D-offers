  ...
  try {
    ...
    const [users, total] = await Promise.all([
      prisma.user.findMany({
        where,
        select: { id: true, name: true, phone: true, pincode: true, city: true, isActive: true, createdAt: true },
  ...
  try {
    ...
    const [coupons, total] = await Promise.all([
      prisma.coupon.findMany({
        where,
        include: { agent: { select: { name: true, phone: true, role: true } } },
        orderBy: { createdAt: 'desc' },
        skip,
        take: parseInt(limit, 10),
      }),
    ...
    res.json({ success: true, data: { coupons, pagination: { total, page: parseInt(page, 10), limit: parseInt(limit, 10), pages: Math.ceil(total / parseInt(limit, 10)) } } });
  } catch (err) {
    next(err);
  }
}
...
async function createCoupon(req, res, next) {
  try {
    const { code, discountType, discountValue, agentId, description, expiryDate, maxUses } = req.body;
    if (!code || !discountType || !discountValue || !agentId) {
      return res.status(400).json({ success: false, message: 'Code, discount type, discount value, and agent ID are required' });
    }
