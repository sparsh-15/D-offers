...
class AiCreditPaymentScreen extends StatefulWidget {
  final Map<String, dynamic> pack;
  final VoidCallback onPaymentSuccess;

  const AiCreditPaymentScreen({
    super.key,
    required this.pack,
  ...
  @override
  State<AiCreditPaymentScreen> createState() => _AiCreditPaymentScreenState();
}

class _AiCreditPaymentScreenState extends State<AiCreditPaymentScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final displayName = widget.pack['displayName'] ?? widget.pack['sku'] ?? 'AI Pack';
    final price = widget.pack['price'] ?? 0;
    final credits = widget.pack['credits'] ?? 0;
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // Pack Summary Card (same style as subscription)
              FadeInDown(
                child: Container(
                  padding: const EdgeInsets.all(20),
                  decoration: BoxDecoration(
  ...
  Future<void> _processPayment() async {
    ...
    try {
      final sku = widget.pack['sku']?.toString() ?? '';
      if (sku.isEmpty) {
        throw Exception('Invalid pack. Please try again.');
      }
      ...
      DialogHelper.showErrorSnackBar(
        context,
        'Payment captured, but pack activation failed. Please try again.',
      );
    }
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'Your AI credit pack has been added successfully.',
